...
class StreamingService implements StreamingController, WebSocketController {
  ...
  Stream<StreamingResponse> addChannel(
    ...
    print("addChannel current active streams: $_activeStreams / $host");
    for (final connection in _connections) {
      print(
          "addChannel current id: ${connection.id} / channel:${connection.channel?.value}");
    }
  ...
  Future<void> removeChannel(String id) async {
    try {
      ...
      print("removeChannel current active streams: $_activeStreams / $host");
      sendRequest(
      ...
      _timer = Timer(Duration(seconds: 5), () async {
        print("Active Streams: $_activeStreams / $host");
        for (final connection in _connections) {
          print("id: ${connection.id} / channel:${connection.channel?.value}");
        }
        if (_activeStreams > 0) {
          return;
        }
